...
{*  Hack para ordenar e incluir en paginador variables GET  *}
{def $uri = $page_uri|explode('?')}
...
<ul class="pagination">
     ...
     {switch match=$:item_previous|lt(0) }
       {case match=0}
      <li><a href={concat($uri[0],$:item_previous|gt(0)|choose('',concat($:offset_text,$:item_previous)),$:view_parameter_text,$page_uri_suffix,'?',$uri[1])|ezurl}>&laquo;&nbsp;{"Previous"|i18n("design/standard/navigator")}</a></li>
       {/case}
       {case match=1}
       {/case}
     {/switch}
    ...
    {switch match=$:item_next|lt($item_count)}
      {case match=1}
        <li><a href={concat($uri[0],$:offset_text,$:item_next,$:view_parameter_text,$page_uri_suffix,'?',$uri[1])|ezurl}>{"Next"|i18n("design/standard/navigator")}&nbsp;&raquo;</a></li>
      {/case}
      {case}
      {/case}
    {/switch}
...
<li><a href={concat($uri[0],$:view_parameter_text,$page_uri_suffix,'?',$uri[1])|ezurl}>1</a></li>
...
{/if}
    ...
    {section loop=$:left_length}
        {let page_offset=sum(sub($ViewParameter:current_page,$ViewParameter:left_length),$:index)}
          <li><a href={concat($uri[0],$:page_offset|gt(0)|choose('',concat($:offset_text,mul($:page_offset,$item_limit))),$ViewParameter:view_parameter_text,$page_uri_suffix,'?',$uri[1])|ezurl}>{$:page_offset|inc}</a></li>
        {/let}
    {/section}

        <li class="active">{$:current_page|inc}</li>
...
<li>...</li>
{/if}
<li><a href={concat($uri[0],$:page_count|dec|gt(0)|choose('',concat($:offset_text,mul($:page_count|dec,$item_limit))),$:view_parameter_text,$page_uri_suffix,'?',$uri[1])|ezurl}>{$:page_count}</a></li>
{/if}
