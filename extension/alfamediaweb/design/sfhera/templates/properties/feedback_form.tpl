...
<form method="post" action={"content/action"|ezurl} class="contact-form" id="form">
	...
	<div class="row-fluid">
		<div class="span6">
			 {attribute_view_gui attribute=$object.data_map.subject placeholder=$object.data_map.subject.contentclass_attribute.name value=concat($node.node_id,'-',$node.name)}
		</div>
	</div>	
	{attribute_view_gui attribute=$object.data_map.message placeholder=$object.data_map.message.contentclass_attribute.name}
	<button type="submit" class="button" name="ActionCollectInformation">{'Send'|i18n('design/alfamediaweb')}</button>
